...
| stock_sub_category_id | UUID | FOREIGN KEY → stock_sub_categories.id, NOT NULL | Required stock sub-category |
...
| invoice_id | UUID | FOREIGN KEY → outcome_invoices.id, NULLABLE | Purchase invoice that created this variant |
...
- Optionally linked to a purchase invoice (outcome_invoices.id) for tracking purchase origin
...
- Stock variants are automatically created when purchase invoices are processed
- Examples:
...
| inventory_category_id | UUID | FOREIGN KEY → stock_categories.id, NULLABLE | Inventory category for classification |
| inventory_sub_category_id | UUID | FOREIGN KEY → stock_sub_categories.id, NULLABLE | Inventory sub-category for classification |
| detail | TEXT | NULLABLE | Item description/notes |
| count | DECIMAL(10,2) | NOT NULL, CHECK (count > 0) | Quantity purchased/sold |
| unit_type | VARCHAR(50) | NOT NULL | Unit type (Kg, g, L, ML, Unit, Box, Bottle, Bag, etc.) |
...
- The invoice type is determined by which table the invoice_id references (no separate invoice_type column needed)
- For outcome invoices: inventory_category_id and inventory_sub_category_id classify what was purchased
- Stock variants reference the parent invoice (not individual items) for purchase origin tracking
...
```
Menu Variants ──< (many) Menu Ingredients >── Stock Sub-Categories
     │                                              │
     └── Casado con Pollo                        └── Pollo (chicken)
     └── Banana Smoothie                          └── Bananas
                                                   └── Leche (milk)
```

### Inventory Hierarchy:
```
Stock Categories (1) ──< (many) Stock Sub-Categories (1) ──< (many) Stock Variants
       │                              │                               │
       └── Drinks                    └── Beers                       └── 6Pack
       └── Alacena                   └── Rice                        └── 1kg
                                      └── Iced Tea                   └── individual
...
- **Menu Ingredients** → **Stock Sub-Categories** (many-to-one, cross-service via API)
