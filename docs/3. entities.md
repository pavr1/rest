# 🗄️ Bar-Restaurant Application - Database Entities
...
| **inventory-service** (8090) | Stock Categories, Stock Sub-Categories, Stock Variants, Stock Count, Suppliers |
...
10. [Stock Variants](#10-stock-variants) - Specific item variations (Wings, Legs, etc.)
11. [Stock Count](#11-stock-count) - Inventory tracking with counts and units
12. [Suppliers](#12-suppliers) - Track stock item suppliers

### Invoice Service Entities
13. [Outcome Invoices](#13-outcome-invoices) - Track expense invoices from suppliers
14. [Income Invoices](#14-income-invoices) - Generate sales invoices for customers
15. [Invoice Items](#15-invoice-items) - Line items for invoices
...
| customer_id | VARCHAR(50) | UNIQUE, NOT NULL | Cédula/identification number |
...
- Flow: Customer scans QR → app asks FIRST if they want loyalty program
  - **If YES**: Customer provides ID, Name (first name + last name), Phone (all required), Email (optional), Date of Birth (optional) → customer record created
  ...
  - **If NO**: Customer enters simple name only → NO customer record created (order uses only person_name)
...
| table_id | UUID | FOREIGN KEY → tables.id, NOT NULL | Table where order was placed |
| customer_id | UUID | FOREIGN KEY → customers.id, NULLABLE | Customer record (only if customer opted into loyalty program) |
...
| promotion_id | UUID | FOREIGN KEY → promotions.id, NULLABLE | Promotion applied to this order |
...
- Flow: App asks FIRST if customer wants loyalty program → if YES, customer provides full info (ID, first+last name, phone) and record created; if NO, customer enters simple name only
- **Status Rules**:
...
- **Promotion Logic**:
  ...
  - **Customer-specific promotions** (`birthday`, `loyalty_reward`): 
    - For `loyalty_reward`: Check if customer has enough `loyalty_points` (must have ≥ `promotion.points_required`)
...
| order_id | UUID | FOREIGN KEY → orders.id, NOT NULL | Parent order |
| menu_variant_id | UUID | FOREIGN KEY → menu_variants.id, NOT NULL | Menu item reference |
...
- **Lost Cost Calculation**: If an order item has `quantity > 1` but only some dishes are lost, `lost_cost` is calculated proportionally:
  - Example: If quantity = 2, unit_price = $10, subtotal = $20, but only 1 dish is lost → `lost_cost` = $10 (half the subtotal, or unit_price × quantity_lost)
...
| category_id | UUID | FOREIGN KEY → menu_categories.id, NOT NULL | Parent menu category |
...
- Examples:
  - Category "Drinks" → Sub-Category "Smoothies" (item_type: bar)
  - Category "Drinks" → Sub-Category "Sodas" (item_type: bar)
  - Category "Desserts" → Sub-Category "Ice Cream" (item_type: kitchen)
...
| sub_category_id | UUID | FOREIGN KEY → menu_sub_categories.id, NOT NULL | Parent sub-category |
...
- Examples:
  - Sub-Category "Smoothies" → Menu Variant "Banana Smoothie" ($5.99)
  - Sub-Category "Smoothies" → Menu Variant "Pineapple Smoothie" ($6.49)
  - Sub-Category "Sodas" → Menu Variant "Coca Cola" ($2.99)

**Menu Hierarchy Example**:
```
Menu Category: Drinks
  └── Menu Sub-Category: Smoothies (item_type: bar)
        ├── Menu Variant: Banana Smoothie ($5.99, prep_time: 5min)
        ├── Menu Variant: Pineapple Smoothie ($6.49, prep_time: 5min)
        └── Menu Variant: Strawberry Smoothie ($5.99, prep_time: 5min)
  └── Menu Sub-Category: Sodas (item_type: bar)
        ├── Menu Variant: Coca Cola ($2.99)
        ├── Menu Variant: Sprite ($2.99)
        └── Menu Variant: Fanta ($2.99)
```

---

### 8. Menu Ingredients
**Purpose**: Define ingredient requirements for menu variants (links menu variants to stock variants)

| Field | Type | Constraints | Description |
|-------|------|-------------|-------------|
| id | UUID | PRIMARY KEY | Unique ingredient requirement identifier |
| menu_variant_id | UUID | FOREIGN KEY → menu_variants.id, NOT NULL | Menu variant that requires this ingredient |
| stock_variant_id | UUID | FOREIGN KEY → stock_variants.id, NOT NULL | Required stock variant |
| quantity | DECIMAL(10,2) | NOT NULL | Quantity required (in standard units) |
...
**Notes**:
- Links individual menu variants to their required stock variants
- `quantity` specifies how much of the stock variant is needed
- Examples:
  - "Casado con Pollo" requires 200g of "Chicken Wings", 150g of "Rice", 100g of "Beans"
  - "Banana Smoothie" requires 2 units of "Banana"
...
- Examples: "Drinks", "Alacena", "Carnes", "Frutas y Verduras", "Bebidas", "Repostería"
...
| description | TEXT | NULLABLE | Sub-category description |
| stock_category_id | UUID | FOREIGN KEY → stock_categories.id, NOT NULL | Parent stock category |
| display_order | INTEGER | NOT NULL, DEFAULT 0 | Order for display in inventory management |
| is_active | BOOLEAN | NOT NULL, DEFAULT true | Whether sub-category is active/visible |
| created_at | TIMESTAMP | NOT NULL, DEFAULT NOW() | Creation timestamp |
| updated_at | TIMESTAMP | NOT NULL, DEFAULT NOW() | Last update timestamp |
...
- Examples:
  - Category "Drinks" → Sub-Category "Beers"
  - Category "Drinks" → Sub-Category "Iced Tea"
  - Category "Alacena" → Sub-Category "Rice"

---

### 10. Stock Variants
**Purpose**: Define specific item variations (e.g., Wings, Legs, Thighs for Chicken)

| Field | Type | Constraints | Description |
|-------|------|-------------|-------------|
| id | UUID | PRIMARY KEY | Unique variant identifier |
| name | VARCHAR(255) | NOT NULL | Variant name (Wings, Legs, Thighs, etc.) |
| description | TEXT | NULLABLE | Variant description |
| stock_sub_category_id | UUID | FOREIGN KEY → stock_sub_categories.id, NOT NULL | Parent sub-category |
| is_active | BOOLEAN | NOT NULL, DEFAULT true | Whether variant is active/available |
| created_at | TIMESTAMP | NOT NULL, DEFAULT NOW() | Creation timestamp |
| updated_at | TIMESTAMP | NOT NULL, DEFAULT NOW() | Last update timestamp |

**Relationships**:
- Belongs to a stock sub-category (stock_sub_categories.id)
- Has many stock_count records (for inventory tracking)
- Used as ingredients in menu variants (menu_ingredients.stock_variant_id)

**Notes**:
- Third level of the inventory hierarchy (under Stock Sub-Categories)
- Defines specific variations of items WITHOUT units or counts (those are in stock_count)
- Stock variants are definitions, not actual inventory
- Examples:
  - Sub-Category "Chicken" → Variants: "Wings", "Legs", "Thighs", "Breast"
  - Sub-Category "Beers" → Variants: "Corona", "Imperial", "Pilsen"
  - Sub-Category "Rice" → Variants: "White Rice", "Brown Rice", "Jasmine Rice"

---

### 11. Stock Count
**Purpose**: Track actual inventory counts for stock variants (created from invoice items)

| Field | Type | Constraints | Description |
|-------|------|-------------|-------------|
| id | UUID | PRIMARY KEY | Unique stock count identifier |
| stock_variant_id | UUID | FOREIGN KEY → stock_variants.id, NOT NULL | The stock variant being counted |
| invoice_id | UUID | FOREIGN KEY → outcome_invoices.id, NOT NULL | Purchase invoice that created this stock |
| count | DECIMAL(10,2) | NOT NULL | Quantity in stock |
| unit | VARCHAR(50) | NOT NULL | Unit of measurement (kg, g, lb, oz, l, ml, unit, dozen, etc.) |
| purchased_at | TIMESTAMP | NOT NULL | When this stock was purchased |
| is_out | BOOLEAN | NOT NULL, DEFAULT false | Whether stock is marked as out (cook reports) |
| created_at | TIMESTAMP | NOT NULL, DEFAULT NOW() | Creation timestamp |
| updated_at | TIMESTAMP | NOT NULL, DEFAULT NOW() | Last update timestamp |

**Unit Values**: `kg`, `g`, `lb`, `oz`, `l`, `ml`, `unit`, `dozen`, `box`, `bottle`, `bag`, etc.

**Relationships**:
- Belongs to a stock variant (stock_variants.id)
- Belongs to an outcome invoice (outcome_invoices.id) - links to supplier via invoice

**Notes**:
- Created automatically when invoice items are processed
- Each record represents a purchase of a specific variant from a specific invoice
- `is_out` is set to true when kitchen staff reports the stock is depleted
- `purchased_at` tracks when the stock was bought (from invoice transaction_date)
- Cost per unit can be calculated from the related invoice item
- Examples:
  - "Wings" → 5 kg purchased on 2026-01-15 from Invoice #123 (Supplier: "Pollos CR")
  - "Corona" → 24 units purchased on 2026-01-14 from Invoice #122 (Supplier: "Cervecería Nacional")
...
| supplier_id | UUID | FOREIGN KEY → suppliers.id, NULLABLE | Supplier reference |
...
**Relationships**:
- Has many invoice_items (via invoice_id)
- Belongs to a supplier (optional)

**Notes**:
- Outcome invoices track stock item purchases from suppliers
- Invoice items can create stock variants when processed
...
|-------|------|-------------|-------------|
| id | UUID | PRIMARY KEY | Unique invoice identifier |
| order_id | UUID | FOREIGN KEY → orders.id, NOT NULL | Related order |
| payment_id | UUID | FOREIGN KEY → payments.id, NULLABLE | Related payment |
| customer_id | VARCHAR(50) | NULLABLE | Customer tax ID (Cédula) - primary customer identifier |
...
| xml_data | TEXT | NULLABLE | XML invoice data (Costa Rica Factura Electrónica) |
...
### 15. Invoice Items
**Purpose**: Line items for invoices (purchased/sold items)
...
| stock_variant_id | UUID | FOREIGN KEY → stock_variants.id, NULLABLE | Stock variant being purchased (for outcome invoices) |
| detail | TEXT | NULLABLE | Item description/notes |
| count | DECIMAL(10,2) | NOT NULL, CHECK (count > 0) | Quantity purchased/sold |
| unit_type | VARCHAR(50) | NOT NULL | Unit type (kg, g, lb, oz, l, ml, unit, dozen, etc.) |
...
| total | DECIMAL(12,2) | GENERATED ALWAYS AS (count × price) STORED | Total cost (count × price) |
| expiration_date | DATE | NULLABLE | Expiration date (if applicable) |
| created_at | TIMESTAMP | NOT NULL, DEFAULT NOW() | Creation timestamp |
| updated_at | TIMESTAMP | NOT NULL, DEFAULT NOW() | Last update timestamp |

**Relationships**:
- Belongs to an invoice (outcome_invoices or income_invoices via invoice_id)
- References a stock variant (for outcome invoices)

**Notes**:
- Invoice items represent individual line items in both income and outcome invoices
- The invoice type is determined by which table the invoice_id references
- For outcome invoices: `stock_variant_id` specifies which variant was purchased
- When an outcome invoice item is created, a `stock_count` record is automatically created with the same count, unit, and invoice reference
- `items_per_unit` allows tracking of sub-units (e.g., 31 ice cream balls per gallon)
- Examples:
  - Invoice item for "Chicken Wings" with stock_variant_id=wings_uuid, count=5, unit_type="kg"
  - Invoice item for "Corona Beer" with stock_variant_id=corona_uuid, count=24, unit_type="unit"
...
| customer_id | UUID | FOREIGN KEY → customers.id, NOT NULL | Customer reference |
| menu_variant_id | UUID | FOREIGN KEY → menu_variants.id, NOT NULL | Menu item reference |
...
| order_id | UUID | FOREIGN KEY → orders.id, NOT NULL | Order being paid |
...
| processed_by_staff_id | UUID | FOREIGN KEY → staff.id, NULLABLE | Staff member who processed payment |
...
| table_id | UUID | FOREIGN KEY → tables.id, NOT NULL | Table making request |
| order_id | UUID | FOREIGN KEY → orders.id, NULLABLE | Related order (if applicable) |
...
| handled_by_staff_id | UUID | FOREIGN KEY → staff.id, NULLABLE | Staff member handling request |
...
| table_id | UUID | FOREIGN KEY → tables.id, NOT NULL | Table requesting song |
...
| skipped_by_staff_id | UUID | FOREIGN KEY → staff.id, NULLABLE | Staff who skipped song |
...
| customer_id | UUID | FOREIGN KEY → customers.id, NOT NULL | Customer earning/spending points |
| order_id | UUID | FOREIGN KEY → orders.id, NULLABLE | Related order (if points from purchase) |
...
| table_id | UUID | FOREIGN KEY → tables.id, NOT NULL | Table for this session |
...
| cleared_by_staff_id | UUID | FOREIGN KEY → staff.id, NULLABLE | Staff member who cleared session |
...
| table_id | UUID | FOREIGN KEY → tables.id, NOT NULL | Reserved table |
| customer_id | UUID | FOREIGN KEY → customers.id, NULLABLE | Customer (if known) |
...
- **Promotion Application Logic**:
  ...
  - **Customer-specific promotions**:
    - `loyalty_reward`: Requires customer to have `loyalty_points` ≥ `points_required`. When applied, points are deducted (negative transaction in `loyalty_points_transactions`)
...
| customer_id | UUID | FOREIGN KEY → customers.id, NULLABLE | Customer (if provided ID/phone) |
| order_id | UUID | FOREIGN KEY → orders.id, NULLABLE | Related order |
| menu_variant_id | UUID | FOREIGN KEY → menu_variants.id, NULLABLE | Item being reviewed |
...
### Menu Hierarchy:
```
Menu Categories (1) ──< (many) Menu Sub-Categories (1) ──< (many) Menu Variants
     │                                 │                              │
     └── Drinks                       └── Smoothies                 └── Banana Smoothie ($5.99)
     └── Desserts                     └── Sodas                     └── Pineapple Smoothie ($6.49)
                                      └── Ice Cream                 └── Coca Cola ($2.99)
```

### Menu-Stock Relationships:
```
Menu Variants ──< (many) Menu Ingredients >── Stock Variants
     │                                              │
     └── Casado con Pollo                        └── Chicken Wings
...
### Inventory Hierarchy:
```
Stock Categories (1) ──< (many) Stock Sub-Categories (1) ──< (many) Stock Variants (1) ──< (many) Stock Count
       │                              │                               │                            │
       └── Meats                     └── Chicken                     └── Wings                   └── 5kg (Invoice #123)
       └── Drinks                    └── Beers                       └── Legs                    └── 3kg (Invoice #124)
                                      └── Rice                       └── Corona                  └── 24 units (Invoice #125)
                                                                     └── Imperial
```
...
- **Order Items** → **Menu Variants** (many-to-one, cross-service via API)
- **Orders** → **Payments** (one-to-many)
- **Tables** → **Request Notifications** (one-to-many)
- **Tables** → **Table Sessions** (one-to-many, active session only)

**Menu Service Relationships:**
- **Menu Categories** → **Menu Sub-Categories** (one-to-many)
- **Menu Sub-Categories** → **Menu Variants** (one-to-many)
- **Menu Variants** → **Menu Ingredients** (one-to-many)
- **Menu Ingredients** → **Stock Variants** (many-to-one, cross-service via API)

**Inventory Service Relationships:**
- **Stock Categories** → **Stock Sub-Categories** (one-to-many)
- **Stock Sub-Categories** → **Stock Variants** (one-to-many)
- **Stock Variants** → **Stock Count** (one-to-many)
- **Stock Count** → **Outcome Invoices** (many-to-one, links to supplier)

**Invoice Service Relationships:**
- **Suppliers** → **Outcome Invoices** (one-to-many)
- **Outcome Invoices** → **Invoice Items** (one-to-many)
- **Orders** → **Income Invoices** (one-to-one, cross-service via API)
- **Payments** → **Income Invoices** (one-to-one, optional)
- **Customers** → **Income Invoices** (one-to-many, optional, cross-service via API)

**Customer Service Relationships:**
- **Customers** → **Orders** (one-to-many, optional, cross-service via API)
- **Customers** → **Customer Favorites** (one-to-many)
- **Menu Variants** → **Customer Favorites** (one-to-many, cross-service via API)
- **Customers** → **Loyalty Points Transactions** (one-to-many)
- **Orders** → **Loyalty Points Transactions** (one-to-many, cross-service via API)

**Promotion Service Relationships:**
- **Orders** → **Promotions** (many-to-one, optional, cross-service via API)
- **Promotions** → **Orders** (one-to-many)

**Karaoke Service Relationships:**
- **Tables** → **Karaoke Song Requests** (one-to-many, cross-service via API)
...
- `menu_ingredients.stock_variant_id`
...
- `stock_count.stock_variant_id` + `stock_count.is_out`
- `stock_count.invoice_id`
...
**Last Updated:** 2026-01-19 (Added stock_count entity, simplified stock_variants, updated invoice_items and menu_ingredients to use stock_variant_id)
