...
| **inventory-service** (8090) | Stock Items, Stock Item Categories, Suppliers, Existences, Ingredients |
| **invoice-service** (8092) | Invoices, Invoice Items, Customer Invoices |
...
14. [Existences](#14-existences) - Track stock item inventory stock and cost information

### Invoice Service Entities
12. [Invoices](#12-invoices) - Track invoices from suppliers
13. [Invoice Items](#13-invoice-items) - Line items for invoices
17. [Invoices (Customer Invoices/Receipts)](#17-invoices-customer-invoicesreceipts) - Generate invoices for transactions
...
### 12. Invoices
**Purpose**: Track invoices from suppliers
...
| invoice_id | UUID | FOREIGN KEY → invoices.id, NOT NULL, ON DELETE CASCADE | Parent invoice |
| stock_item_id | UUID | FOREIGN KEY → stock_items.id, NULLABLE | Stock item reference |
...
### 17. Invoices (Customer Invoices/Receipts)
...
| customer_tax_id | VARCHAR(50) | NULLABLE | Customer tax ID (Cédula) |
...
| xml_data | TEXT | NULLABLE | XML invoice data (for Costa Rica Factura Electrónica) |
...
### 22. Loyalty Points Transactions
...
- **Stock Items** → **Existences** (one-to-many)
- **Invoice Items** → **Existences** (one-to-many, cross-service via API)

**Invoice Service Relationships:**
- **Suppliers** → **Invoices** (one-to-many)
- **Invoices** → **Invoice Items** (one-to-many)
- **Orders** → **Customer Invoices** (one-to-one, cross-service via API)
- **Payments** → **Customer Invoices** (one-to-one, optional)
