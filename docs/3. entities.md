...
| Service | Entities |
|---------|----------|
| **menu-service** (8088) | Menu Categories, Sub Menus, Menu Items |
| **inventory-service** (8090) | Stock Categories, Stock Sub-Categories, Stock Variants, Stock Items, Suppliers, Ingredients |
...
6. [Sub Menus](#6-sub-menus) - Group related menu items (Smoothies, Sodas, etc.)
7. [Menu Items](#7-menu-items) - Actual orderable items with pricing
...
11. [Stock Items](#11-stock-items) - Actual inventory items that can be purchased and tracked
12. [Ingredients](#12-ingredients) - Link menu items to stock items with quantities
13. [Suppliers](#13-suppliers) - Track stock item suppliers
...
| menu_item_id | UUID | FOREIGN KEY → menu_items.id, NOT NULL | Menu item reference |
...
### 5. Sub Menus
**Purpose**: Group related menu items within a category (e.g., Smoothies under Drinks)

| Field | Type | Constraints | Description |
|-------|------|-------------|-------------|
| id | UUID | PRIMARY KEY | Unique sub menu identifier |
| name | VARCHAR(255) | NOT NULL | Sub menu name (e.g., Smoothies, Sodas, Cocktails) |
| description | TEXT | NULLABLE | Sub menu description |
| category_id | UUID | FOREIGN KEY → menu_categories.id, NOT NULL | Parent menu category |
| image_url | VARCHAR(500) | NULLABLE | Sub menu image URL |
| item_type | ENUM | NOT NULL | Item type: kitchen, bar (inherited by all menu items) |
| display_order | INTEGER | NOT NULL, DEFAULT 0 | Order for display in menu |
| is_active | BOOLEAN | NOT NULL, DEFAULT true | Whether sub menu is active/visible |
| created_at | TIMESTAMP | NOT NULL, DEFAULT NOW() | Creation timestamp |
| updated_at | TIMESTAMP | NOT NULL, DEFAULT NOW() | Last update timestamp |
...
- Groups similar menu items together (e.g., all smoothie variants under "Smoothies")
- `item_type` determines which station (kitchen or bar) prepares all items in this sub menu
- Does NOT have pricing information - that's at the Menu Items level
- Examples: 
  - Category "Drinks" → Sub Menu "Smoothies" (item_type: bar)
  - Category "Drinks" → Sub Menu "Sodas" (item_type: bar)
  - Category "Desserts" → Sub Menu "Ice Cream" (item_type: kitchen)
...
| name | VARCHAR(255) | NOT NULL | Item name (e.g., Banana Smoothie, Coca Cola) |
| description | TEXT | NULLABLE | Item description |
| sub_menu_id | UUID | FOREIGN KEY → sub_menus.id, NOT NULL | Parent sub menu |
| price | DECIMAL(10,2) | NOT NULL | Selling price (price customer pays) |
| item_cost | DECIMAL(10,2) | NULLABLE | Item cost (calculated from ingredients) |
...
**Notes**:
- Third and final level of menu hierarchy (under Sub Menus)
- These are the actual items customers order
- `item_type` is inherited from the parent Sub Menu (not stored here)
- `item_cost` is calculated from ingredients (stock items):
  - For each stock item in `menu_item_stock_items`, get the most recent `existence.cost_per_unit`
  - `item_cost` = SUM(existence.cost_per_unit × menu_item_stock_items.quantity) for all stock items
...
- Examples:
  - Sub Menu "Smoothies" → Menu Item "Banana Smoothie" ($5.99)
  - Sub Menu "Smoothies" → Menu Item "Pineapple Smoothie" ($6.49)
  - Sub Menu "Sodas" → Menu Item "Coca Cola" ($2.99)

**Menu Hierarchy Example**:
```
Menu Category: Drinks
  └── Sub Menu: Smoothies (item_type: bar)
        ├── Menu Item: Banana Smoothie ($5.99, prep_time: 5min)
        ├── Menu Item: Pineapple Smoothie ($6.49, prep_time: 5min)
        └── Menu Item: Strawberry Smoothie ($5.99, prep_time: 5min)
  └── Sub Menu: Sodas (item_type: bar)
        ├── Menu Item: Coca Cola ($2.99)
        ├── Menu Item: Sprite ($2.99)
        └── Menu Item: Fanta ($2.99)
```
...
- Examples:
  - Sub-Category "Beers" → Variant "6Pack" (unit: "Bottle", number_of_units: 6)
  - Sub-Category "Beers" → Variant "individual" (unit: "Bottle", number_of_units: 1)
  - Sub-Category "Rice" → Variant "1kg" (unit: "Kg", number_of_units: 1)
  - Sub-Category "Rice" → Variant "8kg" (unit: "Kg", number_of_units: 8)

---

### 11. Stock Items
**Purpose**: Actual inventory items that can be purchased and tracked (references stock variants for specifications)

| Field | Type | Constraints | Description |
|-------|------|-------------|-------------|
| id | UUID | PRIMARY KEY | Unique stock item identifier |
| stock_variant_id | UUID | FOREIGN KEY → stock_variants.id, NOT NULL | Stock variant reference (defines unit and quantity specs) |
| name | VARCHAR(255) | NOT NULL, UNIQUE | Stock item name (specific product name) |
| description | TEXT | NULLABLE | Stock item description |
| invoice_item_id | UUID | FOREIGN KEY → invoice_items.id, NULLABLE | Source invoice item (audit trail) |
| current_stock | DECIMAL(10,2) | NOT NULL, DEFAULT 0 | Current stock quantity |
| unit_cost | DECIMAL(10,2) | NULLABLE | Current unit cost (weighted average) |
| total_value | DECIMAL(12,2) | GENERATED ALWAYS AS (current_stock × unit_cost) STORED | Total current value |
| created_at | TIMESTAMP | NOT NULL, DEFAULT NOW() | Creation timestamp |
| updated_at | TIMESTAMP | NOT NULL, DEFAULT NOW() | Last update timestamp |
...
- `current_stock` tracks available inventory quantity using the unit defined in the stock variant
- `unit_cost` maintains weighted average cost from invoice processing
- `total_value` provides current inventory valuation
- `invoice_item_id` provides audit trail to source invoice
- Stock items are linked to menu items through `menu_item_stock_items` table
- Examples: "Corona Extra", "Arroz Integral", "Leche Entera", "Coca Cola 2L"
...
**Service**: inventory-service (port 8090)

| Field | Type | Constraints | Description |
|-------|------|-------------|-------------|
| id | UUID | PRIMARY KEY | Unique ingredient identifier |
| menu_item_id | UUID | NOT NULL | Menu item reference (from menu-service) |
| stock_item_id | UUID | FOREIGN KEY → stock_items.id, NOT NULL, ON DELETE CASCADE | Stock item reference |
...
- **Managed by inventory-service**: menu-service calls inventory-service API to get/set ingredients
...
- Used to calculate `item_cost` for ALL menu items:
  ...
  - `item_cost` = SUM(existence.cost_per_unit × ingredients.quantity) for all stock items
- Quantity is stored as DECIMAL to support fractional amounts (e.g., 0.5 liters, 1.5 units)
- When menu items are prepared, stock item usage should decrement `current_stock` in the `stock_items` table
- Example: "Bottled Beer" menu item → links to "Bottled Beer" stock item with quantity = 1
...
### 16. Invoice Items
**Purpose**: Line items for invoices (ingredients purchased)
...
| stock_item_id | UUID | FOREIGN KEY → stock_items.id, NULLABLE | Associated stock item |
...
- Used to create/update stock items when outcome invoices are processed
...
| stock_item_id | UUID | FOREIGN KEY → stock_items.id, NOT NULL, ON DELETE CASCADE | Stock item reference |
| invoice_detail_id | UUID | FOREIGN KEY → invoice_details.id, NOT NULL | Invoice detail reference (source of purchase) |
...
| total_purchase_cost | DECIMAL(12,2) | GENERATED ALWAYS AS (units_purchased × cost_per_unit) STORED | Total purchase cost |
| remaining_value | DECIMAL(12,2) | GENERATED ALWAYS AS (units_available × cost_per_unit) STORED | Current inventory value |
| expiration_date | DATE | NULLABLE | Expiration date (if applicable) |
...
**Notes**:
- Stock Items track inventory stock for **ALL items** (raw ingredients, bottled drinks, finished products, etc.)
...
- `remaining_value` tracks current inventory value (units_available × cost_per_unit)
...
### 23. Loyalty Points Transactions
...
| menu_item_id | UUID | FOREIGN KEY → menu_items.id, NULLABLE | Item being reviewed |
...
### Menu Hierarchy:
```
Menu Categories (1) ──< (many) Sub Menus (1) ──< (many) Menu Items
     │                         │                      │
...
### Inventory Hierarchy:
```
Stock Categories (1) ──< (many) Stock Sub-Categories (1) ──< (many) Stock Variants (1) ──< (many) Stock Items
       │                              │                               │                       │
       └── Drinks                    └── Beers                       └── 6Pack                └── Corona Extra 6Pack
       └── Alacena                   └── Rice                        └── 1kg                  └── Arroz Integral 1kg
                                      └── Iced Tea                   └── individual           └── Corona Extra Individual
                                                                    └── 12Pack               └── Coca Cola 2L
```
...
- **Order Items** → **Menu Items** (many-to-one, cross-service via API)
...
- **Menu Categories** → **Sub Menus** (one-to-many)
- **Sub Menus** → **Menu Items** (one-to-many)
...
- **Stock Variants** → **Stock Items** (one-to-many)
- **Menu Items** → **Ingredients** (one-to-many, cross-service via API)
- **Stock Items** → **Ingredients** (one-to-many)
- **Invoice Items** → **Stock Items** (one-to-one, cross-service via API)
...
- **Menu Items** → **Customer Favorites** (one-to-many, cross-service via API)
...
- `sub_menus.category_id` + `sub_menus.is_active`
- `menu_items.sub_menu_id` + `menu_items.is_available`
...
- `stock_items.stock_variant_id`
- `stock_items.name` (for search)
