...
| category_id | UUID | FOREIGN KEY → menu_categories.id, NOT NULL | Menu category |
...
- `item_cost` is the cost to buy/make the item:
  ...
  - Examples:
    - Food item (burger): Uses multiple stock items (bun, patty, lettuce, etc.) → cost calculated from all
    - Bottled beer: Uses single stock item ("Bottled Beer") → cost calculated from that stock item's existence
...
- **Menu Items** → **Menu Categories** (many-to-one)
- **Menu Items** → **Menu Item Stock Items** (one-to-many)
- **Stock Items** → **Menu Item Stock Items** (one-to-many)
