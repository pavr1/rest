...
| **inventory-service** (8090) | Stock Items, Stock Item Categories, Suppliers, Ingredients |
...
8. [Stock Items](#8-stock-items) - Store all items that can be purchased and tracked in inventory
9. [Stock Item Categories](#9-stock-item-categories) - Organize stock items into categories
10. [Ingredients](#10-ingredients) - Link menu items to stock items with quantities
...
| stock_item_category_id | UUID | FOREIGN KEY → stock_item_categories.id, NULLABLE | Stock item category |
...
- `current_stock` tracks available inventory quantity
...
| customer_id | UUID | FOREIGN KEY → customers.id, NULLABLE | Customer reference (if registered) |
| customer_name | VARCHAR(255) | NOT NULL | Customer name (required, editable) |
| customer_tax_id | VARCHAR(50) | NULLABLE | Customer tax ID (Cédula, editable) |
...
**Notes**:
- Customer information (name, tax_id) comes from registered customer session or cashier input
...
| invoice_id | UUID | FOREIGN KEY → outcome_invoices.id, NOT NULL, ON DELETE CASCADE | Parent outcome invoice |
...
**Notes**:
- Invoice items represent individual items purchased in an invoice
- Used to create/update stock items when invoices are processed
...
- **Stock Items** → **Stock Item Categories** (many-to-one)
