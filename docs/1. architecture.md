...
│   │       ├── suppliers/
│   │       ├── existences/
│   │       ├── ingredients/   # Links menu items to stock items
│   │       └── [each entity has:]
│   │           ├── handlers/  # HTTP + DB handlers
│   │           ├── models/    # Domain models
│   │           └── sql/       # Queries & scripts
...
│   │       ├── purchase_invoices/    # Buying from suppliers
│   │       ├── invoice_details/      # Line items for purchases
│   │       ├── customer_invoices/    # Sales receipts
│   │       └── [each entity has:]
│   │           ├── handlers/  # HTTP + DB handlers
│   │           ├── models/    # Domain models
│   │           └── sql/       # Queries & scripts
...
│  │ (auth/JWT)      │ │ (categories, │ │ (orders, items,│        │
│  │                 │ │  sub-menus,  │ │  payments)     │        │
│  │                 │ │  menu items) │ │                │        │
│  └─────────────────┘ └──────────────┘ └────────────────┘        │
│                             │                                   │
...
│                             ▼                                   │
│  ┌──────────────────┐ ┌─────────────────┐                       │
│  │ inventory-service│ │ invoice-service │                       │
...
| 2 | inventory-service | 8090 | data-service | Stock, suppliers,  ingredients |
| 3 | invoice-service | 8092 | data-service, inventory-service | Income & outcome invoices |
...
Level 1:  [data-service]
              │
              ├──────────────────────┬─────────────────────┐
Level 2:  [session-service]    [inventory-service]        │
              │                      │                     │
              │         ┌────────────┴────────────┐        │
Level 3:      │    [invoice-service]    [menu-service]    │
              │         │                   │              │
              │         │    ┌──────────────┘              │
              │         │    │                             │
              │         └────┼─────────────────────────────┘
...
| Inventory Service | 8090 | barrest_inventory_service (future) | Stock Items, Stock Item Categories, Suppliers, Ingredients |
| Invoice Service | 8092 | barrest_invoice_service (future) | Income Invoices, Invoice Items, Outcome Invoices |
