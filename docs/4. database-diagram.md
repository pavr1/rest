...
erDiagram
    ...
    Customers ||--o{ Invoices_Income : "billed_to"
    
    Orders ||--o{ Order_Items : "contains"
    Orders ||--o{ Payments : "has"
    Orders ||--|| Invoices_Income : "generates"
    ...
    Ingredients }o--|| Stock_Variants : "references"
    
    Stock_Categories ||--o{ Stock_Sub_Categories : "contains"
    Stock_Sub_Categories ||--o{ Stock_Variants : "contains"
    Stock_Variants ||--o{ Stock_Count : "tracks"
    Stock_Variants ||--o{ Ingredients : "used_in"
    Stock_Count }o--|| Invoices_Outcome : "purchased_from"

    Suppliers ||--o{ Invoices_Outcome : "supplies"

    Invoices_Outcome ||--o{ Invoice_Items : "contains"
    Invoice_Items }o--|| Stock_Variants : "purchases"
    
    Payments }o--|| Staff : "processed_by"
    Payments }o--|| Invoices_Income : "generates"
...
|---------|------|----------|
| **menu-service** | 8088 | Menu_Categories, Sub_Menus, Menu_Items, Ingredients |
| **inventory-service** | 8090 | Stock_Categories, Stock_Sub_Categories, Stock_Variants, Stock_Count, Suppliers |
| **invoice-service** | 8092 | Invoices_Outcome, Invoice_Items, Invoices_Income |
...
1. **Orders** (orders-service):
   ...
   - Generates Payments and Income Invoices (cross-service)
...
3. **Inventory System** (inventory-service):
   - Stock Categories → Stock Sub-Categories → Stock Variants → Stock Count
   - Stock Variants define WHAT items are (Wings, Legs, Corona, etc.)
   - Stock Count tracks HOW MUCH of each variant is in stock (created from invoice items)
   - Stock Count links to Invoices for supplier traceability

4. **Invoice System** (invoice-service):
   - Invoices: Suppliers → Invoices_Outcome → Invoice_Items
   - Invoice_Items reference Stock_Variants (what was purchased)
   - Creating Invoice_Items automatically creates Stock_Count records
   - Income Invoices: Orders (cross-service) → Invoices_Income
