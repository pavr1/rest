...
erDiagram
    ...
    Menu_Items ||--o{ Menu_Item_Stock_Items : "uses"
    Menu_Items ||--o{ Customer_Favorites : "favorited_by"
    Menu_Items ||--o{ Reviews : "reviewed"
    
    Menu_Item_Stock_Items }o--|| Stock_Items : "references"
    
    Stock_Items }o--|| Stock_Item_Categories : "belongs_to"
    Stock_Items ||--o{ Menu_Item_Stock_Items : "used_in"
...
1. **Orders** are central to the system:
   - Linked to Tables, Customers, Promotions
   - Contains Order Items
   - Generates Payments and Invoices
   - Can generate Loyalty Points Transactions

2. **Menu Hierarchy** (3-tier structure):
   ...
   - Pricing and ingredients are at Menu Item level

3. **Inventory System**:
   - Stock Items → Menu Item Stock Items (Ingredients) → Menu Items
   - Suppliers → Purchase Invoices → Invoice Details → Existences
   - Existences track inventory levels and costs

4. **Customer System**:
   - Customers can have Orders, Favorites, Loyalty Points, Reviews
   - Customer record only created when opting into loyalty program

5. **Promotions**:
   - Applied to Orders (one promotion per order)
