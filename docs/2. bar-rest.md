# 🍻 Bar-Restaurant Application - Feature List
...
## 🎯 Core Customer Features (QR Code Ordering)
  ...
  - When an item is lost (returned, wrong order, quality issue, etc.), staff must:
    ...
    - If restaurant fault:
      - System tracks lost cost (calculated proportionally based on quantity lost)
        - Example: If order item has quantity = 2, but only 1 dish is lost → lost_cost = half the item price
        - Lost cost = (unit_price × quantity_lost) / quantity_ordered
...
- **Menu Item Management**
  ...
  - Set item cost (cost to buy/make the item):
    - **ALL menu items**: Cost is calculated automatically from stock items (sum of stock item costs × quantities)
    - Examples:
...
- **Kitchen Display System (KDS)**
  ...
  - Order status updates (Requested → Preparing → Ready)
...
- **Order Processing**
  ...
  - Update order item status (Requested → Preparing → Ready → Delivered)
  - Time tracking for each status change
...
- **Customer Information Collection**
  - **Flow**: Customer scans QR → app asks if they want loyalty program FIRST
...
- **Stock Tracking (Existences)**
  - **Existences** track stock item inventory:
  ...
  - Track inventory value (`remaining_value` = units_available × cost_per_unit)
...
- **Menu Item Cost Calculation**
  ...
  - Examples:
    - Food item (burger): Uses multiple stock items → cost calculated from all
    - Bottled beer: Uses single stock item ("Bottled Beer") → cost calculated from that stock item's existence
  - Income (profit) = selling price - item cost
  - Cost tracking for financial reporting and pricing decisions
  - When menu items are prepared, stock item usage decrements `units_available` in existences
...
- **Invoice Management**
  ...
  - **Costa Rica Tax Compliance**:
    ...
    - Tax ID (Cédula Jurídica) management
...
- **Promotional Campaigns**
  - **Promotion Types**:
    ...
    - `loyalty_reward`: Customer-specific (requires loyalty points ≥ points_required)
  - **Recurrence Patterns**:
  ...
  - **Promotion Application Logic**:
    ...
    - **Customer-specific promotions**:
      - `loyalty_reward`: Requires customer to have loyalty_points ≥ points_required
...
- **Order Status Updates**
  - Real-time order status changes (Requested → Preparing → Ready → Delivered)
...
- Order accuracy rate
- **Item preparation time** (Requested → Preparing → Ready → Delivered)
...
- **Costa Rica Invoice Requirements**: Need to investigate and comply with:
  ...
  - Electronic invoicing (Factura Electrónica) specifications
...
- **Loyalty Program Opt-in**: App asks FIRST if they want to join loyalty program
  - **If YES**: Must provide ID, Name (first + last name), Phone (all required), Email (optional) → Customer record created
  - **If NO**: Enter simple name only → No customer record created, order proceeds with name only
...
- Track order status in real-time (Requested → Preparing → Ready → Delivered)
...
- Update order item statuses (Requested → Preparing → Ready → Delivered)
...
- **Purchase Invoice Management**: Create purchase invoices from suppliers, add invoice line items with stock item details
- **Inventory Tracking (Existences)**: Track stock item inventory levels, costs, and expiration dates
  - Each existence represents a purchase batch from an invoice
...
- **Loyalty program opt-in flow**: App asks FIRST if customer wants loyalty program → if YES, customer provides ID, Name (first + last), Phone (required), Email (optional) and record created; if NO, customer enters simple name only and no record created
...
- **Item-level status tracking**: Each item has its own status (Requested → Preparing → Ready → Delivered → Lost) with time tracking
...
- **Costa Rica compliance**: Electronic invoicing (Factura Electrónica) support (requires research)
