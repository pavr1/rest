# 🚀 Bar-Restaurant Application - Implementation Plan
...
**Acceptance Criteria**:
- ✅ Database can be started with `docker-compose up`
...
/api/v1/session/*        → session-service
```
...
**Architecture**: Separate microservice (`menu-service/`) with own Docker container.
...
- Image upload and storage (to `ui-service/images/`)

**Technical Specifications**:
- **Cost Calculation**: Real cost from invoice - each existence tracks actual purchase price via `invoice_detail_id → invoice_details.price`. Unit cost = `price / items_per_unit`.
- **Menu Type Filtering**: Exact match on `menu_types` JSONB array.
- **Pagination**: Offset-based (`?page=1&limit=20`) on all list endpoints.
- **Stock Item Units**: Predefined list `["kg", "g", "lb", "oz", "l", "ml", "unit", "dozen"]`.
- **Delete Behavior**: Block with error when dependencies exist (e.g., "Cannot delete category with existing menu items").
- **Authorization**: Gateway validates roles before routing to menu-service.

**Key Features**:
- Menu item endpoints:
  - `GET /api/v1/menu/items` - List menu items (filter by menu_type, category, availability) with pagination
  ...
  - `POST /api/v1/menu/items/:id/image` - Upload menu item image (admin/manager only)
- Menu category endpoints:
  - `GET /api/v1/menu/categories` - List all menu categories
  - `GET /api/v1/menu/categories/:id` - Get category details
  - `POST /api/v1/menu/categories` - Create category (admin/manager only)
  - `PUT /api/v1/menu/categories/:id` - Update category (admin/manager only)
  - `DELETE /api/v1/menu/categories/:id` - Delete category (admin/manager only)
- Stock item category endpoints:
  - `GET /api/v1/stock/categories` - List all stock item categories
  - `GET /api/v1/stock/categories/:id` - Get stock category details
  - `POST /api/v1/stock/categories` - Create stock category (admin/manager only)
  - `PUT /api/v1/stock/categories/:id` - Update stock category (admin/manager only)
  - `DELETE /api/v1/stock/categories/:id` - Delete stock category (admin/manager only)
- Stock item endpoints:
  - `GET /api/v1/stock/items` - List stock items (filter by category)
  - `GET /api/v1/stock/items/:id` - Get stock item details
  - `POST /api/v1/stock/items` - Create stock item (admin/manager only)
  - `PUT /api/v1/stock/items/:id` - Update stock item (admin/manager only)
  - `DELETE /api/v1/stock/items/:id` - Delete stock item (admin/manager only)
- Menu item-stock item relationship endpoints:
  - `GET /api/v1/menu/items/:id/ingredients` - List stock items for a menu item
  - `POST /api/v1/menu/items/:id/ingredients` - Add stock item to menu item (admin/manager only)
  - `PUT /api/v1/menu/items/:id/ingredients/:stockItemId` - Update quantity (admin/manager only)
  - `DELETE /api/v1/menu/items/:id/ingredients/:stockItemId` - Remove stock item from menu item (admin/manager only)
- Cost calculation:
  - `GET /api/v1/menu/items/:id/cost` - Calculate item cost from stock items
  - `POST /api/v1/menu/items/:id/cost/recalculate` - Recalculate and update item_cost (admin/manager only)
...
- ✅ Stock items can be managed
- ✅ Menu item cost calculated from stock items
- ✅ Availability can be toggled
- ✅ Menu filtering by type works

**Nice to Have (Future Enhancements)**:
- Happy hour pricing logic (automatic time-based or manual toggle)
- Dietary tags validation with predefined/managed options
- Allergens validation with predefined/managed options
- Force delete with cascade reassignment option
...
**Purpose**: Manage inventory, suppliers, and stock items.
...
- ✅ Outcome invoices can be created
- ✅ Stock Items updated from invoice processing
...
**Acceptance Criteria**:
- ✅ Customers can be created (loyalty opt-in)
- ✅ Customer lookup works (by ID, phone)
- ✅ Loyalty points tracked correctly
- ✅ Favorites can be added/removed
- ✅ Customer caching works
...
- Order item endpoints:
  ...
  - `PUT /api/v1/orders/:id/items/:item_id/status` - Update item status (Requested → Preparing → Ready → Delivered)
...
- ✅ Order items can be added/removed (only if Requested)
- ✅ Order status updates work
- ✅ Item status tracking works
- ✅ Order splitting works (staff only)
...
**Acceptance Criteria**:
- ✅ Promotions can be created/updated/deleted
- ✅ Time-based promotions checked correctly
- ✅ Customer-specific promotions checked correctly
- ✅ Recurrence patterns work (daily, weekly, monthly)
...
- ✅ Exact change calculated correctly
...
- XML generation (Factura Electrónica)
- Invoice printing
...
- ✅ Invoice data stored correctly
- ⚠️ Costa Rica compliance (pending research)
...
- ✅ Requests can be assigned to staff
...
**Acceptance Criteria**:
- ✅ Song requests can be created
- ✅ FIFO queue works correctly
- ✅ Songs cannot be reordered
- ✅ Song library can be managed
- ✅ Song status tracking works
...
Gateway Service ──┐
    ...
    ├──→ Promotion Service
    │       ↓
    │   Orders Service ──→ Payment Service ──→ Invoice Service
    │
    ├──→ Request Notification Service
    │
    ├──→ Karaoke Service
    │
    └──→ UI Service (depends on all via Gateway)
```
...
- ✅ UI Service (Customer interface)
- ✅ UI Service (Staff dashboard)
- ✅ UI Service (Admin panel)
