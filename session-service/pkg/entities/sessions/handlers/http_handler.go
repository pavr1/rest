...
func (h *HTTPHandler) CreateSession(w http.ResponseWriter, r *http.Request) {
	...
	if req.Username == "" || req.Password == "" {
		...
		sharedHttp.SendError(w, http.StatusBadRequest, "Username and password are required", nil)
		return
	}
	...
	if err != nil {
		...
		sharedHttp.SendError(w, http.StatusUnauthorized, "Invalid username or password", nil)
		return
	}

	h.logger.WithField("username", req.Username).Info("Login successful")
	sharedHttp.SendSuccess(w, http.StatusCreated, "Login successful", response)
}

func (h *HTTPHandler) ValidateSession(w http.ResponseWriter, r *http.Request) {
	var req models.SessionValidationRequest
	if err := json.NewDecoder(r.Body).Decode(&req); err != nil {
		sharedHttp.SendError(w, http.StatusBadRequest, "Invalid request", err)
		return
	}
	...
	sharedHttp.SendSuccess(w, http.StatusOK, "Session validated", response)
}
